#python OOSv12.tuning.py -f="jkp_ff_monthly.csv" -p="JKPFF" -w=120 -t=50 -q=1000 -m="NNKeras" 
...
# function to generate random fourier features for input data
def func_generate_RFF(i_X, i_rff_dim):  # inputs are X-array and number of dimensions for fourier features
  N, D = i_X.shape  # number of rows (N) and columns (D) in the input data

  # generate random variables for RFF creation (seed)
  W = np.random.normal(loc=0, scale=1, size=(i_rff_dim, D))
  b = np.random.uniform(0, 2*np.pi, size=i_rff_dim)
  B    = np.repeat(b[:, np.newaxis], N, axis=1)

  norm = 1./ np.sqrt(i_rff_dim)

  Z    = norm * np.sqrt(2) * np.cos(W @ i_X.T + B) # calculate RFFs with matrix multiplication
  RFF_X = Z.T #transpose the result to get correct dimensionality
...
def tune_ts_model (i_splits, i_model_prefix, i_tune_frequency = 12, i_model_type = "RF", i_trials = 25, i_PCA=False, i_RFF=False):
  ...
  #l_split = splits[1]
    ...
    if i_RFF == True:
      # generate raondom fourier features
      df_input_ = func_generate_RFF(df_input_,10000)
...
parser.add_argument('-r','--rffadjust', default=False, action="store_true", help='default no Random Fourier Feature adjustment, generate RFFs by setting this flag')
...
if pcaadjust_: output_prefix_=output_prefix_+"_PCAadjusted"
if rffadjust_: output_prefix_=output_prefix_+"_RFFadjusted"
